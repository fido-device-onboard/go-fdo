...
func RunServerStateSuite(t *testing.T, state AllServerState) { //nolint:gocyclo
	...
	t.Run("OwnerVoucherPersistentState", func(t *testing.T) {
		...
		if _, err := state.Voucher(t.Context(), newGUID); !errors.Is(err, fdo.ErrNotFound) {
			t.Fatalf("new voucher GUID should return not found, got error %v", err)
		}
