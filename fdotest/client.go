...
func RunClientTestSuite(t *testing.T, conf Config) {
	...
	// This subtest is needed to wait for all parallel table tests to complete
			...
			t.Run(fmt.Sprintf("Key %q Encoding %q Exchange %q Cipher %q", table.keyType, table.keyEncoding, table.keyExchange, table.cipherSuite), func(t *testing.T) {
				...
				transport := &Transport{
					...
					DIResponder: &fdo.DIServer[custom.DeviceMfgInfo]{
						Session:  state,
						Vouchers: state,
						DeviceInfo: func(context.Context, *custom.DeviceMfgInfo, []*x509.Certificate) (string, protocol.KeyType, protocol.KeyEncoding, error) {
