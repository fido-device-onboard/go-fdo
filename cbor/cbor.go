...
type StreamMarshaler interface {
	...
	// than zero, in which case it must decode exactly that many items or fail.
	//
...
func (d *Decoder) Decode(v any) error {
	...
	for rv := reflect.ValueOf(v); (rv.Kind() == reflect.Pointer || rv.Kind() == reflect.Interface) && !rv.IsNil(); rv = rv.Elem() {
		...
		if u, ok := rv.Interface().(StreamUnmarshaler); ok {
			//FIXME: Limit reader to size
			if err := u.UnmarshalCBORStream(io.LimitReader(io.MultiReader(
				d.r,
			), 1000), d.DecoderOptions, 0); !errors.Is(err, ErrSkip) {
				return err
			}
		}
...
func (d *Decoder) decodeStructField(rv reflect.Value, idx []int) error {
	...
	if n, ok := flatN(rv.Type().FieldByIndex(idx)); ok {
		...
		if !ok {
			panic("struct field with cbor flat option must implement StreamUnmarshaler")
		}
...
func (e *Encoder) encodeStruct(size int, get func([]int) reflect.Value, field func([]int) reflect.StructField) error {
	...
	for i, idx := range indices {
		...
		if n, ok := flatN(field(idx)); ok {
			...
			if !ok {
				panic("struct field with cbor flat option must implement StreamMarshaler")
			}
