...
// Note: TestClientWithMockModuleAndAutoUnchunkingFDO20 is not included because
// the "device module did not read full body" error detection is specific to
// the 1.0.1 client's streaming message body handling. The 2.0 client uses
// fully-buffered messages (bytes.NewReader) so this check doesn't apply.
